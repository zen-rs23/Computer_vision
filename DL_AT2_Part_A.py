...
Original file is located at
    https://colab.research.google.com/drive/128jMW-_r2JgwllzMR9ZVawSNr0mMJCkZ
"""
...
image_size = 224 # Define the size of the images after resizing

# Series of transformations to apply to the training images
...
# Series of transformations to apply to the testing images
transform_test = transforms.Compose([
    transforms.Resize(size=(image_size, image_size)),  # Resize the image to the specified size
    transforms.CenterCrop(224),  # Center crop the image to get a square of size 100x100
    transforms.ToTensor(),  # Convert the image to a PyTorch tensor
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),  # Normalize the image
])
...
val_size = int(0.2 * len(train_dataset)) # Calculating the size of the validation set
...
googlenet_model.avgpool = nn.AdaptiveAvgPool2d((1, 1))

num_ftrs = googlenet_model.fc.in_features  # Get the number of input features for the fully connected layer
googlenet_model.fc = nn.Sequential(  # Replace the fully connected layer with a new sequential module
    nn.Flatten(),  # Flatten the output of the pooling layer
    nn.Linear(num_ftrs, 512),  # Add a new fully connected layer with 512 output features
    nn.ReLU(),  # Apply ReLU activation function
    nn.Linear(512, 256),  # Add another fully connected layer with 256 output features
    nn.ReLU(),  # Apply ReLU activation function
    nn.Linear(256, 101),  # Final fully connected layer with 101 output features for Food101 dataset
    nn.Softmax(dim=1)  # Apply softmax activation function along the second dimension
)
...
optimizer = torch.optim.Adam(googlenet_model.parameters(), lr=0.0001)
...
for epoch in range(num_epochs):  # Iterate over each epoch
    googlenet_model.train()               # Set the model to train mode
    ...
    for inputs, l,labels in tqdm(train_loader,total=len(train_loader)):
        ...
        # Inside your training loop, just before passing inputs to the model                        # Debugging print
        #print(f"Input shape before model: {inputs.shape}")                                         # Debugging print

        #if len(inputs.shape) == 3:  # Assuming missing channel dimension for grayscale images      # Debugging print
          #inputs = inputs.unsqueeze(1)  # Add channel dimension                                    # Debugging print
          #print(f"Input shape after unsqueeze: {inputs.shape}")  # Confirm shape                   # Debugging print
...
mobilenet_v3_model.classifier[0].in_features  # Get the number of input features for the fully connected layer
...
mobilenet_v3_model.classifier = nn.Sequential(
    nn.Linear(mobilenet_v3_model.classifier[0].in_features, 512),
    nn.ReLU(),
    nn.Dropout(0.2),
    nn.Linear(512, 256),
    nn.ReLU(),
...
# Define the optimizer (Adam) for updating the model parameters during training
optimizer = optim.Adam(mobilenet_v3_model.parameters(), lr=0.00001)
...
for epoch in range(num_epochs):  # Iterate over each epoch
    ...
    # Disable gradient calculation for validation
    with torch.no_grad():
        for inputs, l, labels in val_loader:
            inputs, labels = inputs.to(device), labels.to(device)  # Move inputs and labels to device
            outputs = mobilenet_v3_model(inputs)  # Forward pass
            labels_one_hot = F.one_hot(labels.long(), num_classes=101).float()
            loss = criterion(outputs.squeeze(), labels_one_hot) # Compute the validation loss
